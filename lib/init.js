...
var initBase = function (type, server, callback) {
  var db = new Db();
  var shard = require('./baseshard/index.js');
  if (type === 'cas') shard = require('./baseshard/cas.js');
  async.each(shard.routes, function (path, callback) {
    var route = new Route(path);
...
module.exports = function (type, server, callback) {
  async.parallel([
    function (callback) {
      var list = [];
      ...
      initBase(type, server, function (err) {
        return callback(err);
      });
  }], function (err) {
    return callback(err);
